...
if [ -f "/usr/local/devpod/config/initial-spec.json" ]; then
  gcloud iam workload-identity-pools create-cred-config \
    projects/184212049955/locations/global/workloadIdentityPools/devpod-flex/providers/devpod-flex-provider \
    --service-account=preview-environmnet-dev@devpod-dev-preview.iam.gserviceaccount.com \
    --service-account-token-lifetime-seconds=1h \
    --output-file="${PREVIEW_ENV_DEV_SA_KEY_PATH}" \
    --executable-command='node /workspace/devpod/dev/flex-oidc/oidc.js' \
