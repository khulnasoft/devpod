...
while [ "$documentIndex" -le "$DOCS" ]; do
   ...
   if [[ "blobserve" == "$NAME" ]] && [[ "$KIND" == "ConfigMap" ]]; then
      ...
      jq 'if .blobserve.repos["eu.gcr.io/devpod-core-dev/build/ide/code"] == null then .blobserve.repos["eu.gcr.io/devpod-core-dev/build/ide/code"] = .blobserve.repos["eu.gcr.io/devpod-dev-artifact/build/ide/code"] else . end' /tmp/"$NAME"-overrides.json > /tmp/"$NAME"-updated-overrides.json
