...
import (
	...
	"github.com/khulnasoft/devpod/previewctl/pkg/k8s"
	kctx "github.com/khulnasoft/devpod/previewctl/pkg/k8s/context"
	pssh "github.com/khulnasoft/devpod/previewctl/pkg/ssh"
)
...
func (k *ConfigLoader) Load(ctx context.Context) (*api.Config, error) {
	if k.client == nil {
		...
		err = k.connectToHost(ctx, fmt.Sprintf("%s.preview.devpod-dev.com", k.opts.PreviewName), "2222")
		if err != nil {
			k.logger.Error(err)
			return nil, err
		}
...
func (k *ConfigLoader) getContext(ctx context.Context) (*api.Config, error) {
	...
	k3sConfig.Clusters[k.opts.PreviewName].Server = fmt.Sprintf("https://%s.preview.devpod-dev.com:6443", k.opts.PreviewName)
