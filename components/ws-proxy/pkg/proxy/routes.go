...
import (
	...
	"github.com/khulnasoft/devpod/ws-manager/api"
...
func installWorkspacePortRoutes(r *mux.Router, config *RouteHandlerConfig, infoProvider common.WorkspaceInfoProvider) error {
	...
	r.NewRoute().HandlerFunc(
		func(rw http.ResponseWriter, r *http.Request) {
			// a work-around for servers which does not respect case-insensitive headers, see https://github.com/khulnasoft/devpod/issues/4047#issuecomment-856566526
		...
		return http.HandlerFunc(func(resp http.ResponseWriter, req *http.Request) {
			...
			for _, c := range cookies {
				...
				if cookie == "" {
					...
					// See https://github.com/khulnasoft/devpod/issues/2470 for more details.
					var (
						originalName    = c.Name
			...
			// being sent. See https://github.com/khulnasoft/devpod/issues/2121.
