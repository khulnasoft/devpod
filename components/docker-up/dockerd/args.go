...
func convertRemapUser(arg string, value interface{}) ([]string, error) {
	...
	return []string{"--userns-remap", "devpod"}, nil
}
...
func adaptSubid(oldfile string, id int) error {
	...
	if id != 0 {
		newfile.WriteString(mappingFmt("devpod", 1, id))
		newfile.WriteString(mappingFmt("devpod", devpodUserId, 1))
	} else {
		newfile.WriteString(mappingFmt("devpod", devpodUserId, 1))
		newfile.WriteString(mappingFmt("devpod", 1, devpodUserId-1))
		newfile.WriteString(mappingFmt("devpod", devpodUserId+1, 32200)) // map rest of user ids in the user namespace
	}

	uidScanner := bufio.NewScanner(uid)
	for uidScanner.Scan() {
		l := uidScanner.Text()
		if !strings.HasPrefix(l, "devpod") {
