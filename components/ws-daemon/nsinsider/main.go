...
import (
	...
	_ "github.com/khulnasoft/devpod/common-go/nsenter"
		...
		Commands: []*cli.Command{
			...
			},
			{
				...
				Action: func(c *cli.Context) error {
					...
					// nft add table ip devpod
					devpodTable := nftcon.AddTable(&nftables.Table{
						Family: nftables.TableFamilyIPv4,
						Name:   "devpod",
					})

					// nft add chain ip devpod ratelimit { type filter hook postrouting priority 0 \; }
					...
					// nft add counter devpod connection_drop_stats
					nftcon.AddObject(&nftables.CounterObj{
						Table: devpodTable,
						Name:  drop_stats,
					})

					// nft add set devpod ws-connections { type ipv4_addr; flags timeout, dynamic; }
					set := &nftables.Set{
						Table:      devpodTable,
						Name:       "ws-connections",
					...
					// nft add rule ip devpod ratelimit ip protocol tcp ct state new meter ws-connections
