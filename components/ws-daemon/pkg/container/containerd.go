...
func (s *Containerd) GetContainerImageInfo(ctx context.Context, id ID) (*workspacev1.WorkspaceImageInfo, error) {
	...
	if manifest.Annotations != nil {
		wsImageInfo.WorkspaceImageRef = manifest.Annotations["io.devpod.workspace-image.ref"]
		if size, err := strconv.Atoi(manifest.Annotations["io.devpod.workspace-image.size"]); err == nil {
