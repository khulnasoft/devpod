...
import (
	...
	api "github.com/khulnasoft/devpod/ide-metrics-api"
...
func NewGrpcMetricsReporter(devpodHost string) *GrpcMetricsReporter {
	return &GrpcMetricsReporter{
		...
		addCounter: func(name string, labels map[string]string, value uint64) {
			doAddCounter(devpodHost, name, labels, value)
		},
		addHistogram: func(name string, labels map[string]string, count uint64, sum float64, buckets []uint64) {
			doAddHistogram(devpodHost, name, labels, count, sum, buckets)
		},
...
func doAddCounter(devpodHost string, name string, labels map[string]string, value uint64) {
	...
	url := fmt.Sprintf("https://ide.%s/metrics-api/metrics/counter/add/%s", devpodHost, name)
...
// TODO(ak) refactor to use grpc when ide-proxy supports it
func doAddHistogram(devpodHost string, name string, labels map[string]string, count uint64, sum float64, buckets []uint64) {
	...
	url := fmt.Sprintf("https://ide.%s/metrics-api/metrics/histogram/add/%s", devpodHost, name)
	ctx, cancel := context.WithTimeout(context.Background(), time.Second*5)
	defer cancel()
	request, err := http.NewRequestWithContext(ctx, http.MethodPost, url, bytes.NewReader(body))
	if err != nil {
