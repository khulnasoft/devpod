...
type APIInterface interface {
	GetToken(ctx context.Context, query *devpod.GetTokenSearchOptions) (res *devpod.Token, err error)
	OpenPort(ctx context.Context, port *devpod.WorkspaceInstancePort) (res *devpod.WorkspaceInstancePort, err error)
	UpdateGitStatus(ctx context.Context, status *devpod.WorkspaceInstanceRepoStatus) (err error)
	WorkspaceUpdates(ctx context.Context) (<-chan *devpod.WorkspaceInstance, error)
...
type Service struct {
	...
	subs     map[chan *devpod.WorkspaceInstance]struct{}
	subMutex sync.Mutex
...
func NewServerApiService(ctx context.Context, cfg *ServiceConfig, tknsrv api.TokenServiceServer) *Service {
	...
	service := &Service{
		...
		subs:       make(map[chan *devpod.WorkspaceInstance]struct{}),
	}
...
func (s *Service) GetToken(ctx context.Context, query *devpod.GetTokenSearchOptions) (res *devpod.Token, err error) {
	if s == nil {
...
func (s *Service) UpdateGitStatus(ctx context.Context, status *devpod.WorkspaceInstanceRepoStatus) (err error) {
	if s == nil {
		return errNotConnected
	}
	startTime := time.Now()
...
func (s *Service) OpenPort(ctx context.Context, port *devpod.WorkspaceInstancePort) (res *devpod.WorkspaceInstancePort, err error) {
	if s == nil {
		return nil, errNotConnected
	}
	startTime := time.Now()
	...
	// see https://github.com/khulnasoft/devpod/blob/2967579c330de67090d975661a6e3e1cd970ab68/components/server/src/workspace/devpod-server-impl.ts#L1521
	return port, nil
}
...
func (s *Service) WorkspaceUpdates(ctx context.Context) (<-chan *devpod.WorkspaceInstance, error) {
	if s == nil {
		return nil, errNotConnected
	}
	ch := make(chan *devpod.WorkspaceInstance)
	s.subMutex.Lock()
	s.subs[ch] = struct{}{}
	s.subMutex.Unlock()
...
func workspaceStatusToWorkspaceInstance(status *v1.WorkspaceStatus) *devpod.WorkspaceInstance {
	instance := &devpod.WorkspaceInstance{
		CreationTime: status.Instance.CreatedAt.String(),
		ID:           status.Instance.InstanceId,
		Status: &devpod.WorkspaceInstanceStatus{
			ExposedPorts: []*devpod.WorkspaceInstancePort{},
	...
	for _, port := range status.Instance.Status.Ports {
		info := &devpod.WorkspaceInstancePort{
			Port: float64(port.Port),
			URL:  port.Url,
		}
		if port.Policy == v1.PortPolicy_PORT_POLICY_PUBLIC {
			info.Visibility = devpod.PortVisibilityPublic
		} else {
			info.Visibility = devpod.PortVisibilityPrivate
		}
		if port.Protocol == v1.PortProtocol_PORT_PROTOCOL_HTTPS {
			info.Protocol = devpod.PortProtocolHTTPS
		} else {
			info.Protocol = devpod.PortProtocolHTTP
		}
