...
func AddGitpodUserIfNotExists() error {
	ok, err := hasGroup(devpodGroupName, devpodGID)
	if err != nil {
		return err
	}
	if !ok {
		err = addGroup(devpodGroupName, devpodGID)
		if err != nil {
			return err
		}
	}
	if err := addSudoer(devpodGroupName); err != nil {
		log.WithError(err).Error("add devpod sudoers")
	}

	targetUser := &user.User{
		Uid:      strconv.Itoa(devpodUID),
		Gid:      strconv.Itoa(devpodGID),
		Username: devpodUserName,
		HomeDir:  "/home/" + devpodUserName,
	}
...
func hasGroup(name string, gid int) (bool, error) {
	...
	if grpByID == nil && grpByName != nil {
		...
		return true, xerrors.Errorf("group named %s exists but uses different GID %s, should be: %d", name, grpByName.Gid, devpodGID)
	}
...
func hasUser(u *user.User) (bool, error) {
	...
	if userByID == nil && userByName != nil {
		...
		return true, xerrors.Errorf("user named %s exists but uses different UID %s, should be: %d", u.Username, userByName.Uid, devpodUID)
	}
...
func addSudoer(group string) error {
	...
	devpodSudoer := []byte(fmt.Sprintf("%%%s ALL=NOPASSWD:ALL", group))
	// Line starts with "%devpod ..."
	re := regexp.MustCompile(fmt.Sprintf("(?m)^%%%s\\s+.*?$", group))
	if len(re.FindStringIndex(string(b))) > 0 {
		nb := re.ReplaceAll(b, devpodSudoer)
