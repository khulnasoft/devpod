...
type GitTokenProvider struct {
	...
	devpodAPI           serverapi.APIInterface
}

// NewGitTokenProvider creates a new instance of gitTokenProvider.
func NewGitTokenProvider(devpodAPI serverapi.APIInterface, workspaceConfig WorkspaceConfig, notificationService *NotificationService) *GitTokenProvider {
...
func (p *GitTokenProvider) GetToken(ctx context.Context, req *api.GetTokenRequest) (tkn *Token, err error) {
	...
	token, err := p.devpodAPI.GetToken(ctx, &devpod.GetTokenSearchOptions{
		Host: req.Host,
	})
	if err != nil {
		return nil, err
...
type gitStatusUpdateContext struct {
	lastSuccessfullStatus *devpod.WorkspaceInstanceRepoStatus
...
func (s *GitStatusService) Run(ctx context.Context, wg *sync.WaitGroup) {
	...
	go func() {
		updates, err := s.devpodService.WorkspaceUpdates(ctx)
		if err != nil {
...
func (s *GitStatusService) update(ctx context.Context, updateContext *gitStatusUpdateContext) {
	...
	var newStatus *devpod.WorkspaceInstanceRepoStatus
	if status != nil {
		...
		newStatus = &devpod.WorkspaceInstanceRepoStatus{
	...
	err = s.devpodService.UpdateGitStatus(ctx, newStatus)
	if err != nil {
