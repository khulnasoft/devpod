...
func prepareSSHKey(ctx context.Context, sshkey string) error {
	...
	err = os.Chown(sshkey, devpodUID, devpodGID)
	if err != nil {
...
func ensureSSHDir(cfg *Config) error {
	...
	_ = exec.Command("chown", "-R", fmt.Sprintf("%d:%d", devpodUID, devpodGID), d).Run()
...
func configureSSHDefaultDir(cfg *Config) {
	...
	file, err := os.OpenFile("/home/devpod/.bash_profile", os.O_WRONLY|os.O_APPEND|os.O_CREATE, 0o644)
	if err != nil {
