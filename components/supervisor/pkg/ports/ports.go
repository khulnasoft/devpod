...
func (pm *Manager) nextState(ctx context.Context) map[uint32]*managedPort {
	...
	genManagedPort := func(port uint32) *managedPort {
		...
		var portConfig *devpod.PortConfig
	...
	for _, exposed := range pm.exposed {
		...
		if exposed.Protocol == devpod.PortProtocolHTTPS {
...
// deprecated
func getOnExposedAction(config *devpod.PortConfig, port uint32) api.OnPortExposedAction {
	if config == nil {
...
func getOnOpenAction(config *devpod.PortConfig, port uint32) api.PortsStatus_OnOpenAction {
	if config == nil {
		// anything above 32767 seems odd (e.g. used by language servers)
		unusualRange := !(0 < port && port < 32767)
		wellKnown := port <= 10000
