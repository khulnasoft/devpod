...
<fg=ff971d>  +***+-.      .-+**** </>      devpod workspace create   <lightgray>Create a new workspace</>
<fg=ff971d>  .=*****+=::-+*****+: </>      devpod workspace open     <lightgray>Open a running workspace</>
<fg=ff971d>  .:=+*********=-.     </>      devpod workspace stop     <lightgray>Stop a running workspace</>
<fg=ff971d>      .-++++=:         </>
...
func getGitpodClient(ctx context.Context) (*client.Gitpod, error) {
	...
	if host == nil {
		return nil, prettyprint.AddResolution(fmt.Errorf("active context has no host configured"),
			"set a host using `devpod config set-context --current --host <host>`",
