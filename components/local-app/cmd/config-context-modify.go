...
var configContextModifyCmd = &cobra.Command{
	...
	Short:   "Modifies a context entry in the devpod CLI config",
		...
		if configContextModifyOpts.Current {
			if len(args) > 0 {
				return prettyprint.AddResolution(fmt.Errorf("cannot use --current and specify a context name"),
					"modify current context with `{devpod} config set-context --current`",
					"modify/create a different context with `{devpod} config set-context <name>`",
				)
			}
