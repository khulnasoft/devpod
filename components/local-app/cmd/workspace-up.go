...
var workspaceUpCmd = &cobra.Command{
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		if workspaceCreateOpts.WorkspaceClass != "" {
			resp, err := devpod.Workspaces.ListWorkspaceClasses(cmd.Context(), connect.NewRequest(&v1.ListWorkspaceClassesRequest{}))
			if err != nil {
		...
		if workspaceCreateOpts.Editor != "" {
			resp, err := devpod.Editors.ListEditorOptions(cmd.Context(), connect.NewRequest(&v1.ListEditorOptionsRequest{}))
			if err != nil {
		...
		for {
			...
			if parentDir == currentDir {
				...
				return prettyprint.AddResolution(fmt.Errorf("no Git repository found"),
					...
					"open a remote repository using `{devpod} workspace create <repo-url>`",
				)
			}
		...
		ws, err := helper.ObserveWorkspaceUntilStarted(ctx, devpod, workspaceID)
		if err != nil {
			return err
		}
		slog.Debug("workspace started", "workspaceID", workspaceID)

		token, err := devpod.Workspaces.GetOwnerToken(ctx, connect.NewRequest(&v1.GetOwnerTokenRequest{WorkspaceId: workspaceID}))
		if err != nil {
			return err
		}
		var (
		...
		pushcmd := exec.Command("git", "push", "--progress", "devpod")
