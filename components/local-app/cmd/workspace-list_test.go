	...
	RunCommandTests(t, []CommandTest{
		...
		},
		{
			...
			PrepServer: func(mux *http.ServeMux) {
				mux.Handle(devpod_experimental_v1connect.NewWorkspacesServiceHandler(&testWorkspaceListCmdWorkspaceSrv{
...
type testWorkspaceListCmdWorkspaceSrv struct {
	Resp *v1.ListWorkspacesResponse
	Err  error
	devpod_experimental_v1connect.UnimplementedWorkspacesServiceHandler
}
