...
import (
	...
	app "github.com/khulnasoft/devpod/local-app/api"
	...
	for _, ws := range s.workspaces {
		...
		cfg.Hosts = append(cfg.Hosts, &ssh_config.Host{
			...
			Nodes: []ssh_config.Node{
				...
				&ssh_config.KV{Key: "User", Value: "devpod"},
...
func New(client devpod.APIInterface, localAppTimeout time.Duration, cb Callbacks) *Bastion {
...
func (b *Bastion) FullUpdate() {
	wss, err := b.Client.GetWorkspaces(b.ctx, &devpod.GetWorkspacesOptions{Limit: float64(100)})
	if err != nil {
...
func generateSSHKeys(instanceID string) (privateKeyFN string, publicKey string, err error) {
	privateKeyFN = filepath.Join(os.TempDir(), fmt.Sprintf("devpod_%s_id_rsa", instanceID))
	useRrandomFile := func() {
		var tmpf *os.File
		tmpf, err = ioutil.TempFile("", "devpod_*_id_rsa")
		if err != nil {
			return
		}
		tmpf.Close()
...
func (b *Bastion) connectTunnelClient(ctx context.Context, ws *Workspace) error {
	...
	h := make(http.Header)
	h.Set("x-devpod-owner-token", ws.OwnerToken)
	webSocket := devpod.NewReconnectingWebsocket(tunnelURL, h, logrus.WithField("workspace", ws.WorkspaceID))
...
func newTunnelClient(ctx context.Context, ws *Workspace, reconnecting *devpod.ReconnectingWebsocket) (client *TunnelClient, closed chan struct{}, err error) {
	logrus.WithField("workspace", ws.WorkspaceID).Info("tunnel: trying to connect ssh client...")
	err = reconnecting.EnsureConnection(func(conn *devpod.WebsocketConnection) (bool, error) {
