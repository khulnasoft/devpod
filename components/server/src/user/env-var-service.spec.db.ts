...
const barUserAnotherCommitEnvVar = {
    name: "bar",
    value: "commit",
    repositoryPattern: "devpod/openvscode-server",
} as const;
...
describe("EnvVarService", async () => {
    ...
    it("should allow updating org env vars by owner", async () => {
        const added = await es.addOrgEnvVar(owner.id, org.id, devpodImageAuthOrgEnvVar);
    ...
    it("user should have precedence over org, project over user", async () => {
        ...
        expectEnvVars(envVars, [devpodImageAuthOrgEnvVar]);
