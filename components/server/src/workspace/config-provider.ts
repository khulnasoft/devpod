...
export class ConfigProvider {
    ...
    public async fetchCustomConfig(
        ...
        try {
            ...
            if (AdditionalContentContext.is(commit) && commit.additionalFiles[".devpod.yml"]) {
                customConfigString = commit.additionalFiles[".devpod.yml"];
            ...
            return { customConfig, configBasePath, literalConfig: { ".devpod.yml": customConfigString || "" } };
        } catch (e) {
            TraceContext.setError({ span }, e);
            throw e;
        } finally {
    ...
    private async validateConfig(config: WorkspaceConfig, user: User): Promise<void> {
        ...
        if (checkoutLocation) {
            ...
            if (this.leavesWorkspaceBase(normalizedPath)) {
                ...
                throw new Error(
                    `Checkout location must not leave the ${POD_PATH_WORKSPACE_BASE} folder. Check your .devpod.yml file.`,
                );
            }
        }
        ...
        if (workspaceLocation) {
            ...
            if (this.leavesWorkspaceBase(normalizedPath)) {
                ...
                throw new Error(
                    `Workspace location must not leave the ${POD_PATH_WORKSPACE_BASE} folder. Check your .devpod.yml file.`,
                );
            }
        }
    }
