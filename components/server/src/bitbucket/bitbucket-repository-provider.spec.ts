...
class TestBitbucketRepositoryProvider {
    ...
    @test public async testGetRepo() {
        const result = await this.repoProvider.getRepo(this.user, "devpod", "integration-tests");
        expect(result).to.deep.include({
            ...
            webUrl: "https://bitbucket.org/devpod/integration-tests",
        });
    }
    ...
    @test public async testHasReadAccess_positive() {
        const result = await this.repoProvider.hasReadAccess(this.user, "devpod", "integration-tests");
    ...
    @test public async testSearchRepos_matchesSubstring() {
        ...
        expect(result).to.deep.include({
            url: "https://bitbucket.org/devpod-integration-tests/exampul-repo",
            name: "exampul-repo",
        });
    }
    ...
    @test public async testSearchRepos_doesNotMatchPath() {
        const result = await this.repoProvider.searchRepos(this.user, "devpod-integration-tests/exampul-repo", 100);
