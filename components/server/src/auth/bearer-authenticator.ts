...
export class BearerAuth {
        ...
        return async (req, res, next) => {
            try {
                ...
                if (isBearerAuthError(e)) {
                    // (AT) while investigating https://github.com/khulnasoft/devpod/issues/8703 we
    ...
    private async userAndScopesFromToken(token: string): Promise<{ user: User; scopes: string[] }> {
        ...
        //  1. Personal Access Tokens which are prefixed with `devpod_pat_`
...
export class PersonalAccessToken {
    ...
    private static PAT_PREFIX = "devpod_pat_";
