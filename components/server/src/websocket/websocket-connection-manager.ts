...
export type WebsocketClientType =
    ...
    | "supervisor"
    | "local-companion"
    | "io.devpod.jetbrains.remote"
    | "io.devpod.jetbrains.gateway";
    ...
    export function getClientType(req: express.Request): WebsocketClientType | undefined {
        ...
        if (userAgent) {
            if (userAgent.startsWith("Go-http-client")) {
                ...
                result = "supervisor";
            } else if (userAgent.startsWith("devpod/local-companion")) {
                result = "local-companion";
            } else if (userAgent === "io.devpod.jetbrains.remote" || userAgent === "io.devpod.jetbrains.gateway") {
                result = userAgent;
            }
        }
        if (result === undefined) {
...
export class WebsocketConnectionManager implements ConnectionHandler {
    ...
    protected createProxyTarget(
        ...
        const devpodServer = this.serverFactory();
        ...
        client.onDidCloseConnection(() => {
            try {
                devpodServer.dispose();
                increaseApiConnectionClosedCounter();
                this.events.emit(EVENT_CONNECTION_CLOSED, devpodServer, expressReq);

                clientContext.removeEndpoint(devpodServer);
        ...
        clientContext.addEndpoint(devpodServer);
