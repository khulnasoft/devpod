...
export const mockAuthConfig: AuthConfig = {
    ...
    session: {
        issuer: "https://mp-server-d7650ec945.preview.devpod-dev.com",
        lifetimeSeconds: 7 * 24 * 60 * 60,
        cookie: {
            name: "__Host-_devpod_dev_jwt_",
            secure: true,
            httpOnly: true,
            maxAge: 7 * 24 * 60 * 60,
            sameSite: "lax",
...
const mockApplyingContainerModule = new ContainerModule((bind, unbound, isbound, rebind) => {
    ...
    rebind(IDEServiceDefinition.name).toConstantValue(
        createMock(<Partial<IDEServiceClient>>{
            async resolveWorkspaceConfig() {
                return {
                    envvars: [],
                    supervisorImage: "devpod/supervisor:latest",
                    webImage: "devpod/code:latest",
                    ideImageLayers: [],
                    refererIde: "code",
                    ideSettings: "",
                    tasks: "",
    ...
    rebind<Partial<Config>>(Config).toConstantValue({
        ...
        workspaceDefaults: {
            workspaceImage: "devpod/workspace-full",
            defaultFeatureFlags: [],
            previewFeatureFlags: [],
        },
        workspaceClasses: [
