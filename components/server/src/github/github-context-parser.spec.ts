...
class TestGithubContextParser {
    ...
    @test public async testTreeContext_04() {
        const result = await this.parser.handle(
            {},
            this.user,
            "https://github.com/khulnasoft/devpod-test-repo/blob/nametest/src/src/server.ts",
        );
        expect(result).to.deep.include({
            ref: "nametest/src",
            refType: "branch",
            ...
            title: "khulnasoft/devpod-test-repo - nametest/src",
        });
    }
    ...
    @test public async testTreeContext_05() {
        const result = await this.parser.handle(
            {},
            this.user,
            "https://github.com/khulnasoft/devpod-test-repo/tree/499efbbcb50e7e6e5e2883053f72a34cd5396be3/folder1/folder2",
        );
        expect(result).to.deep.include({
    ...
    @test public async testReleasesContext_tag_01() {
        const result = await this.parser.handle(
            {},
            this.user,
            "https://github.com/khulnasoft/devpod/releases/tag/v0.9.0",
        );
        expect(result).to.deep.include({
            ref: "v0.9.0",
            refType: "tag",
            isFile: false,
            path: "",
            title: "khulnasoft/devpod - v0.9.0",
    ...
    @test public async testCommitsContext_01() {
        const result = await this.parser.handle(
            {},
            this.user,
            "https://github.com/khulnasoft/devpod-test-repo/commits/4test",
        );
        expect(result).to.deep.include({
            ref: "4test",
            refType: "branch",
            ...
            title: "khulnasoft/devpod-test-repo - 4test",
        });
    }
    ...
    @test public async testCommitContext_01() {
        const result = await this.parser.handle(
            {},
            this.user,
            "https://github.com/khulnasoft/devpod-test-repo/commit/409ac2de49a53d679989d438735f78204f441634",
        );
        expect(result).to.deep.include({
            ref: "",
            refType: "revision",
            ...
            title: "khulnasoft/devpod-test-repo - Test 3",
        });
    }
    ...
    @test public async testCommitContext_02_notExistingCommit() {
        try {
            await this.parser.handle(
                {},
                this.user,
                "https://github.com/khulnasoft/devpod-test-repo/commit/aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa",
            );
    ...
    @test public async testCommitContext_02_invalidSha() {
        try {
            await this.parser.handle({}, this.user, "https://github.com/khulnasoft/devpod-test-repo/commit/invalid");
            // ensure that an error has been thrown
            chai.assert.fail();
        } catch (e) {
    ...
    @test public async testIssueContext_01() {
        const result = await this.parser.handle(
            {},
            this.user,
            "https://github.com/khulnasoft/devpod-test-repo/issues/42",
        );
        expect(result).to.deep.include({
    ...
    @test public async testIssuePageContext() {
        const result = await this.parser.handle({}, this.user, "https://github.com/khulnasoft/devpod-test-repo/issues");
        expect(result).to.deep.include({
    ...
    @test public async testIssueThroughPullRequestContext() {
        const result = await this.parser.handle(
            {},
            this.user,
            "https://github.com/khulnasoft/devpod-test-repo/pull/42",
        );
        expect(result).to.deep.include({
            title: "Test issue web-extension",
            repository: {
    ...
    @test public async testBlobContext_01() {
        const result = await this.parser.handle(
            {},
            this.user,
            "https://github.com/khulnasoft/devpod-test-repo/blob/aba298d5084a817cdde3dd1f26692bc2a216e2b9/test-comment-01.md",
        );
        expect(result).to.deep.include({
    ...
    @test public async testBlobContext_02() {
        const result = await this.parser.handle(
            {},
            this.user,
            "https://github.com/khulnasoft/devpod-test-repo/blob/499efbbcb50e7e6e5e2883053f72a34cd5396be3/folder1/folder2/content2",
        );
        expect(result).to.deep.include({
    ...
    @test public async testBlobContextShort_01() {
        const result = await this.parser.handle(
            {},
            this.user,
            "https://github.com/khulnasoft/devpod-test-repo/blob/499efbbc/folder1/folder2/content2",
        );
        expect(result).to.deep.include({
            title: "khulnasoft/devpod-test-repo - 499efbbc:folder1/folder2/content2",
            repository: {
    ...
    @test public async testBlobContextShort_02() {
        const result = await this.parser.handle(
            {},
            this.user,
            "https://github.com/khulnasoft/devpod-test-repo/blob/499ef/folder1/folder2/content2",
        );
        expect(result).to.deep.include({
            title: "khulnasoft/devpod-test-repo - 499efbbc:folder1/folder2/content2",
            repository: {
