...
class TestAzureDevOpsContextParser {
    ...
    @test public async testBranch() {
        ...
        const result2 = await this.parser.handle(
            {},
            this.user,
            "https://dev.azure.com/services-azure/test-project/_git/repo2?path=/.devpod.yml&version=GBdevelop-2&_a=contents",
        );
    ...
    @test public async testTag() {
        ...
        const result2 = await this.parser.handle(
            {},
            this.user,
            "https://dev.azure.com/services-azure/test-project/_git/repo2-fork?version=GTv0.0.1&path=/.devpod.yml",
        );
        expect(result2).to.deep.include({
            path: "",
            isFile: false,
    ...
    @test public async testCommit() {
        ...
        expect(result).to.deep.include({
            ...
            isFile: false,
            title: "test-project/repo2-fork - Updated .devpod.yml",
            owner: "services-azure/test-project",
            repository: {
                host: "dev.azure.com",
                owner: "services-azure/test-project",
        ...
        const result3 = await this.parser.handle(
            {},
            this.user,
            "https://dev.azure.com/services-azure/test-project/_git/repo2-fork/commit/4c47246a2eacd9700aab401902775c248e85aee7?refName=refs/heads/develop&path=/.devpod.yml",
        );
        expect(result3).to.deep.include({
            path: "",
            ref: "",
