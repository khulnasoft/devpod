...
import (
	...
	supervisorapi "github.com/khulnasoft/devpod/supervisor/api"
)
				...
				if ferr, ok := err.(*jsonrpc2.Error); ok && ferr.Code == http.StatusForbidden && setEnvScope == envScopeUser {
					...
					if ferr, ok := err.(*jsonrpc2.Error); ok && ferr.Code == http.StatusForbidden {
						...
						return fmt.Errorf(""+
							...
							"Please create the var manually under %s/user/variables using Name=%s, Scope=*/**, Value=foobar", v.Name, result.devpodHost, v.Name)
					}
				} else {
					return err
				}
