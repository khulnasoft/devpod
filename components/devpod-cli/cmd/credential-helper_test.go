	...
	tests := []struct {
		...
		},
		{
			...
			Commands: []string{
				...
				"/usr/lib/git-core/git remote-https origin https://github.com/jeanp413/test-private-package.git",
				"git --no-replace-objects clone https://github.com/jeanp413/test-private-package.git /home/devpod/.npm/_cacache/tmp/git-cloneUsL7Mf --recurse-submodules --depth=1",
			},
		...
		},
		{
			...
			Commands: []string{
				"/usr/lib/git-core/git remote-https origin https://github.com/devpod-samples/spring-petclinic.git",
				"/bin/git -c core.quotepath=false -c log.showSignature=false push --progress --porcelain origin refs/heads/master:master",
			},
			Expected: gitCommandInfo{RepoUrl: "https://github.com/devpod-samples/spring-petclinic.git", GitCommand: "push"},
		},
	}
