...
import (
	...
	"github.com/khulnasoft/devpod/devpod-cli/pkg/supervisor"
...
func runRebuild(ctx context.Context, supervisorClient *supervisor.SupervisorClient) error {
	...
	if devpodConfig == nil {
		fmt.Println("To test the image build, you need to configure your project with a .devpod.yml file")
		fmt.Println("")
	...
	switch img := devpodConfig.Image.(type) {
	case nil:
		...
		if image == "" {
			image = "devpod/workspace-full:latest"
		}
	...
	case map[interface{}]interface{}:
		...
		if _, err := os.Stat(dockerfilePath); os.IsNotExist(err) {
			fmt.Println("Your .devpod.yml points to a Dockerfile that doesn't exist: " + dockerfilePath)
	...
	if err != nil {
		dockerPath = "/.supervisor/devpod-docker-cli"
	}
	...
	workspaceLocation := devpodConfig.WorkspaceLocation
	...
	tasks, err := json.Marshal(devpodConfig.Tasks)
	if err != nil {
		return err
	}
	...
	prepareFS := []mnte{
		...
		{Source: "/workspace/.devpod-debug/.docker-root", Target: "/workspace/.docker-root", Permission: 0710},
		{Source: "/workspace/.devpod-debug/.devpod", Target: "/workspace/.devpod", Permission: 0751},
		{Source: "/workspace/.devpod-debug/.vscode-remote", Target: "/workspace/.vscode-remote", Permission: 0751},
		{Source: "/workspace/.devpod-debug/.cache", Target: "/workspace/.cache", Permission: 0751},
		{Source: "/workspace/.devpod-debug/.config", Target: "/workspace/.config", Permission: 0751},
	...
	for _, mnt := range prepareFS {
		...
		if mnt.Target == "" {
			...
			// if target is not same with source and it not a file, ensure target is created by devpod user
...
func init() {
	...
	setFlags := func(cmd *cobra.Command) {
		...
		_ = cmd.PersistentFlags().MarkHidden("devpod-env")
