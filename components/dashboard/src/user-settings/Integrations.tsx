...
function GitProviders() {
    ...
    const disconnect = async (ap: AuthProviderDescription) => {
        ...
        const returnTo = devpodHostUrl.with({ pathname: "complete-auth", search: "message=success" }).toString();
        const deauthorizeUrl = devpodHostUrl
            .withApi({
...
export function GitIntegrationModal(
    ...
    const callbackUrl = useMemo(() => devpodHostUrl.with({ pathname: `/auth/callback` }).toString(), []);
