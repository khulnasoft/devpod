...
const getInstallerVersion = async (version: string | undefined) => {
    ...
    try {
        tagInfo =
            await $`git ls-remote --tags --sort=-v:refname https://github.com/khulnasoft/devpod | grep ${v} | head -n1`.text();
    } catch (e) {
...
export const getIDEVersionOfImage = async (img: string) => {
    console.log(
        ...
        `oci-tool fetch image ${img} | jq -r '.config.Labels["io.devpod.ide.version"]'`,
    );
    const version = await $`oci-tool fetch image ${img} | jq -r '.config.Labels["io.devpod.ide.version"]'`
        .text()
        .catch((e) => {
