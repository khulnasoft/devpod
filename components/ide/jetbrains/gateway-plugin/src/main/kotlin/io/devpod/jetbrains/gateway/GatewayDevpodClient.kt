...
class GatewayGitpodClient(
    ...
    private fun scheduleTimeout(reason: String) {
        ...
        timeoutJob = GlobalScope.launch {
            thisLogger().info("$devpodHost: connection times out in $timeoutDelayInMinutes minutes: $reason")
    ...
    private fun cancelTimeout(reason: String) {
        if (timeoutJob?.isActive == true) {
            thisLogger().info("$devpodHost: canceled connection timeout: $reason")
            timeoutJob!!.cancel()
        }
    }
        ...
        syncJob = GlobalScope.launch {
            for (id in listeners.keys) {
                ...
                try {
                    ...
                    thisLogger().error("$devpodHost: $id: failed to sync", t)
                }
            }
        }
    }
