...
class GitpodWebSocketTunnelServer(
    ...
    fun start(lifetime: Lifetime) {
        val job = GlobalScope.launch(Dispatchers.IO) {
            thisLogger().info("devpod: tunnel[$url]: listening on port $port")
            try {
                while (isActive) {
                    try {
                        ...
                        if (isActive) {
                            thisLogger().error("devpod: tunnel[$url]: failed to accept", t)
                        }
                    }
                }
            } catch (t: Throwable) {
                if (isActive) {
                    thisLogger().error("devpod: tunnel[$url]: failed to listen", t)
                }
            } finally {
                thisLogger().info("devpod: tunnel[$url]: stopped")
            }
        }
    ...
    private fun handleClientConnection(clientSocket: Socket) {
        ...
        try {
            ...
            socketClient.onMessageCallback = { data ->
                ...
                thisLogger().trace("devpod: tunnel[$url]: received ${data.size} bytes")
            }
            ...
            while (inputStream.read(buffer).also { read = it } != -1) {
                ...
                thisLogger().trace("devpod: tunnel[$url]: sent $read bytes")
            }
        } catch (t: Throwable) {
            if (t is SocketException && t.message?.contains("Socket closed") == true) {
                return
            }
            thisLogger().error("devpod: tunnel[$url]: failed to pipe", t)
        } finally {
    ...
    private fun connectToWebSocket(socketClient: GitpodWebSocketTunnelClient) {
        ...
        for (proxy in proxies) {
            ...
            if (proxyAddress !is InetSocketAddress) {
                thisLogger().warn("devpod: tunnel[$url]: unexpected proxy: $proxy")
                continue
            }
        ...
        val config = ClientEndpointConfig.Builder.create()
            .configurator(object : Configurator() {
                override fun beforeRequest(headers: MutableMap<String, List<String>>) {
                    headers["x-devpod-owner-token"] = Collections.singletonList(ownerToken)
                }
            })
            .build()
...
class GitpodWebSocketTunnelClient(
    ...
    override fun onClose(session: Session, closeReason: CloseReason) {
        thisLogger().info("devpod: tunnel[$url]: closed ($closeReason)")
        this.doClose()
    }

    override fun onError(session: Session?, thr: Throwable?) {
        thisLogger().error("devpod: tunnel[$url]: failed", thr)
        this.doClose()
    }
    ...
    private fun doClose() {
        try {
            ...
            thisLogger().error("devpod: tunnel[$url]: failed to close socket", t)
        }
        try {
            container?.stop()
        } catch (t: Throwable) {
            thisLogger().error("devpod: tunnel[$url]: failed to stop container", t)
        }
    }
    ...
    fun close() {
        try {
            ...
            thisLogger().error("devpod: tunnel[$url]: failed to close", t)
        }
        try {
            container?.stop()
        } catch (t: Throwable) {
