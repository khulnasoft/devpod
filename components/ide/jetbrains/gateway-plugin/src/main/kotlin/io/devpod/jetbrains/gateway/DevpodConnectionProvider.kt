...
class GitpodConnectionProvider : GatewayConnectionProvider {
    ...
    override suspend fun connect(
        ...
        var connectionKeyId =
            "${connectParams.devpodHost}-${connectParams.resolvedWorkspaceId}-${connectParams.backendPort}"
        ...
        val client = devpod.obtainClient(connectParams.devpodHost)
        ...
        GlobalScope.launch {
            ...
            if (settings.additionalHeartbeat) {
                thisLogger().info("devpod: additional heartbeat enabled for ${connectParams.resolvedWorkspaceId}")
                    ...
                    while (isActive) {
                        ...
                        if (thinClientJob?.isActive == true) {
                            try {
                                ...
                                thisLogger().error(
                                    "devpod: failed to send additional heartbeat for ${connectParams.resolvedWorkspaceId}",
                                    t
                                )
                            }
                        } else {
                            thisLogger().debug("devpod: thinClient is not active, skipping additional heartbeat for ${connectParams.resolvedWorkspaceId}")
                        }
            ...
            try {
                for (update in updates) {
                    try {
                        ...
                        if (thinClientJob == null && update.status.phase == "running") {
                            thinClientJob = launch thinClientJob@{
                                try {
                                    ...
                                    if (credentials == null) {
                                        setErrorMessage("${connectParams.devpodHost} installation does not allow SSH access")
                                        ...
                                        backendStatusJob = launch backendStatusJob@{
                                            while (isActive) {
                                                try {
                                                    ...
                                                    thisLogger().error(
                                                        "${connectParams.devpodHost}: ${connectParams.resolvedWorkspaceId}: failed to reconnect:",
                                                        t
                                                    )
                                                }
                                            }
                                    ...
                                    thisLogger().error(
                                        "${connectParams.devpodHost}: ${connectParams.resolvedWorkspaceId}: failed to connect:",
                                        t
                                    )
                        ...
                        thisLogger().error(
                            "${connectParams.devpodHost}: ${connectParams.resolvedWorkspaceId}: failed to process workspace update:",
                            e
                        )
                    }
                }
                ...
                thisLogger().error(
                    "${connectParams.devpodHost}: ${connectParams.resolvedWorkspaceId}: failed to process workspace updates:",
                    t
                )
    ...
    private suspend fun resolveCredentialsWithWebSocketTunnel(
        ...
        try {
            ...
            thisLogger().error(
                "${connectParams.devpodHost}: web socket tunnel: failed to connect:",
                t
            )
            return null
        }
    ...
    private suspend fun resolveCredentialsWithDirectSSH(
        ...
        if (hostKeys.isNullOrEmpty()) {
            thisLogger().error("${connectParams.devpodHost}: direct SSH: failed to resolve host keys for")
            return null
        }
        ...
        try {
            ...
            thisLogger().error(
                "${connectParams.devpodHost}: direct SSH: failed to resolve credentials",
                t
            )
            return null
        }
        ...
        if (resp.statusCode != 200) {
            if (!resp.body.isNullOrBlank() && resp.body.contains("not implemented")) {
                if (!sendHeartBeatThroughSupervisorLogOnce) {
                    thisLogger().warn("devpod: sendHeartbeat ${connectParams.actualWorkspaceId} failed: method is not implemented in supervisor")
                    sendHeartBeatThroughSupervisorLogOnce = true
                }
                return
            }
            thisLogger().error("devpod: sendHeartbeat ${connectParams.actualWorkspaceId} failed: ${resp.statusCode}, body: ${resp.body}")
            return
        }
        thisLogger().debug("devpod: sendHeartbeat succeed for ${connectParams.actualWorkspaceId}")
    }
    ...
    private suspend fun fetchWS(
        ...
        if (!ownerToken.isNullOrBlank()) {
            httpRequestBuilder = httpRequestBuilder.header("x-devpod-owner-token", ownerToken)
        }
        ...
        while (true) {
            ...
            try {
                ...
                if (response.statusCode() < 500) {
                    thisLogger().error("${connectParams.devpodHost}: ${connectParams.resolvedWorkspaceId}: failed to fetch '$endpointUrl': ${response.statusCode()}")
                    return null
                }
                thisLogger().warn("${connectParams.devpodHost}: ${connectParams.resolvedWorkspaceId}: failed to fetch '$endpointUrl', trying again...: ${response.statusCode()}")
            } catch (t: Throwable) {
                if (t is CancellationException) {
                    throw t
                }
                thisLogger().warn(
                    "${connectParams.devpodHost}: ${connectParams.resolvedWorkspaceId}: failed to fetch '$endpointUrl', trying again...:",
                    t
                )
            }
    ...
    override fun isApplicable(parameters: Map<String, String>): Boolean =
        parameters.containsKey("devpodHost")

    data class ConnectParams(
        val devpodHost: String,
        ...
        val title = "$resolvedWorkspaceId ($devpodHost)"
    }
