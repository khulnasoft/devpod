...
import io.devpod.supervisor.api.*
import io.devpod.supervisor.api.Info.WorkspaceInfoResponse
import io.devpod.supervisor.api.Notification.*
...
class GitpodManager : Disposable {
    ...
    init {
        ...
        // rate(devpod_jb_backend_low_memory_after_gc_total[5m])
        val lowMemoryCounter = Counter.build()
            .name("devpod_jb_backend_low_memory_after_gc")
    ...
    init {
        val monitoringJob = GlobalScope.launch {
            ...
            // max_over_time(devpod_jb_backend_memory_used_bytes[5m:])/max_over_time(devpod_jb_backend_memory_max_bytes[5m:])
            val allocatedGauge = Gauge.build()
                .name("devpod_jb_backend_memory_max_bytes")
                .help("Total allocated memory of JB backend in bytes.")
                .labelNames("product", "qualifier")
                .register(registry)
            val usedGauge = Gauge.build()
                .name("devpod_jb_backend_memory_used_bytes")
            ...
            while (isActive) {
                ...
                try {
                    ...
                    thisLogger().error("devpod: failed to push monitoring metrics:", t)
                }
                delay(5000)
            }
        }
    ...
    init {
        GlobalScope.launch {
            ...
            try {
                ...
                thisLogger().error("devpod: failed to resolve gateway link:", t)
            }
        }
    }
    ...
    private val notificationsJob = GlobalScope.launch {
        ...
        while (isActive) {
            try {
                ...
                thisLogger().error("devpod: failed to stream notifications: ", t)
            }
            delay(1000L)
        }
    }
    ...
    private val serverJob = GlobalScope.launch {
        ...
        val connect = {
            ...
            try {
                val proxies = CommonProxy.getInstance().select(URL(info.devpodHost))
        ...
        val devpodHost = info.devpodApi.host
        ...
        try {
            while (kotlin.coroutines.coroutineContext.isActive) {
                try {
                    ...
                    thisLogger().warn("$devpodHost: connection closed, reconnecting after $reconnectionDelay milliseconds: $closeReason")
                } catch (t: Throwable) {
        ...
        thisLogger().warn("$devpodHost: connection permanently closed: $closeReason")
    }
    init {
        lifetime.onTerminationOrNow {
            serverJob.cancel()
    ...
    private val metricsJob = GlobalScope.launch {
        ...
        while (isActive) {
            try {
                ...
                thisLogger().error("devpod: failed to retrieve resource status: ", t)
            }
            delay(1000L)
        }
    }
    ...
    fun openUrlFromAction(url: String) {
        trackEvent("jb_execute_command_devpod_open_link", mapOf("url" to url))
