...
import io.devpod.supervisor.api.Status.PortsStatus
...
abstract class AbstractGitpodPortForwardingService : GitpodPortForwardingService {
    ...
    private fun start() {
        ...
        if (isLocalPortForwardingDisabled()) {
            thisLogger().warn("devpod: Local port forwarding is disabled.")
        }
            ...
            try {
                ...
                when (throwable) {
                    is InterruptedException, is CancellationException -> {
                        cancel("devpod: Stopped observing ports list due to an expected interruption.")
                    }

                    else -> {
                        thisLogger().warn(
                            "devpod: Got an error while trying to get ports list from Supervisor. " +
    ...
    private fun observePortsList(): CompletableFuture<Void> {
        ...
        val portsStatusResponseObserver = object :
            ...
            override fun beforeStart(request: ClientCallStreamObserver<Status.PortsStatusRequest>) {
                lifetime.onTerminationOrNow { request.cancel("devpod: Service lifetime terminated.", null) }
            }
    ...
    private fun startForwarding(portStatus: PortsStatus) {
        ...
        try {
            ...
            if (throwable !is PortAlreadyForwardedException) {
                thisLogger().warn("devpod: Caught an exception while forwarding port: ${throwable.message}")
            }
        }
    }
