...
import io.devpod.devpodprotocol.api.entities.WorkspaceInstancePort
import io.devpod.supervisor.api.Info
import io.devpod.supervisor.api.Status
import io.devpod.supervisor.api.Status.PortVisibility
...
abstract class AbstractGitpodClientProjectSessionTracker(private val project: Project) : Disposable {
    ...
    private suspend fun makePortPublic(workspaceId: String, port: PortsStatus) {
        ...
        p.visibility = io.devpod.devpodprotocol.api.entities.PortVisibility.PUBLIC.toString()
        p.url = port.exposed.url

        try {
            manager.client.server.openPort(workspaceId, p).await()
        } catch (e: Exception) {
            thisLogger().error("devpod: failed to open port ${port.localPort}: ", e)
        }
    }
    ...
    private val portsObserveJob = GlobalScope.launch {
        ...
        while (isActive) {
            try {
                ...
                thisLogger().error("devpod: failed to stream ports status: ", t)
            }
            delay(1000L)
        }
    }
