...
import (
	...
	"github.com/khulnasoft/devpod/jetbrains/launcher/pkg/constant"
...
func isBackendPluginReady(ctx context.Context, backendPort string, shouldWaitBackendPlugin bool) error {
	...
	url, err := url.Parse("http://localhost:" + backendPort + "/api/devpod/cli?op=metrics")
	if err != nil {
		return err
	}
...
func resolveToolboxLink(wsInfo *supervisor.WorkspaceInfoResponse) (string, error) {
	...
	link := url.URL{
		Scheme:   "jetbrains",
		Host:     "gateway",
		Path:     "io.devpod.toolbox.gateway/open-in-toolbox",
		RawQuery: fmt.Sprintf("host=%s&workspaceId=%s&debugWorkspace=%t", devpodUrl.Hostname(), wsInfo.WorkspaceId, debugWorkspace),
	}
...
func resolveGatewayLink(backendPort string, wsInfo *supervisor.WorkspaceInfoResponse) (string, error) {
	...
	link := url.URL{
		...
		Host:     "connect",
		Fragment: fmt.Sprintf("devpodHost=%s&workspaceId=%s&backendPort=%s&debugWorkspace=%t", devpodUrl.Hostname(), wsInfo.WorkspaceId, backendPort, debugWorkspace),
	}
	return link.String(), nil
}
...
func resolveJsonLink(backendPort string) (string, error) {
	var (
		hostStatusUrl = "http://localhost:" + backendPort + "/codeWithMe/unattendedHostStatus?token=devpod"
...
func terminateIDE(backendPort string) error {
	var (
		hostStatusUrl = "http://localhost:" + backendPort + "/codeWithMe/unattendedHostStatus?token=devpod&exit=true"
...
func launch(launchCtx *LaunchContext) {
	...
	err = configureVMOptions(devpodConfig, launchCtx.alias, launchCtx.vmOptionsFile)
	if err != nil {
	...
	err = installPlugins(devpodConfig, launchCtx)
	...
	// install devpod plugin
	err = linkRemotePlugin(launchCtx)
	if err != nil {
		log.WithError(err).Error("failed to install devpod-remote plugin")
	}
...
func updateVMOptions(
	...
	if err == nil && parsedMemory > 0 {
		...
		if xmx > minDefaultXmx {
			devpodVMOptions = append(devpodVMOptions, fmt.Sprintf("-Xmx%dm", int(xmx)))
		}
	}

	vmoptions := deduplicateVMOption(ideaVMOptionsLines, devpodVMOptions, filterFunc)
	...
	// project-defined vmoptions (.devpod.yml)
	if config != nil {
...
func collectSyncSources(launchCtx *LaunchContext, target SyncTarget) ([]string, error) {
	...
	for _, srcDir := range []string{
		fmt.Sprintf("%s/.devpod/jetbrains/%s", userHomeDir, target),
		fmt.Sprintf("%s/.devpod/jetbrains/%s/%s", userHomeDir, launchCtx.alias, target),
		fmt.Sprintf("%s/.devpod/jetbrains/%s", launchCtx.projectDir, target),
		fmt.Sprintf("%s/.devpod/jetbrains/%s/%s", launchCtx.projectDir, launchCtx.alias, target),
	} {
...
func getProductConfig(config *devpod.GitpodConfig, alias string) *devpod.JetbrainsProduct {
	...
	productConfig, ok := v.(*devpod.JetbrainsProduct)
	if !ok {
		return nil
	}
...
func linkRemotePlugin(launchCtx *LaunchContext) error {
	...
	remotePluginDir := remotePluginsFolder + "/devpod-remote"
	if err := os.MkdirAll(remotePluginsFolder, 0755); err != nil {
		return err
	}

	// added for backwards compatibility, can be removed in the future
	sourceDir := "/ide-desktop-plugins/devpod-remote-" + os.Getenv("JETBRAINS_BACKEND_QUALIFIER")
	_, err := os.Stat(sourceDir)
	if err == nil {
		return safeLink(sourceDir, remotePluginDir)
	}

	return safeLink("/ide-desktop-plugins/devpod-remote", remotePluginDir)
}
