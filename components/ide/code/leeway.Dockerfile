...
FROM devpod/openvscode-server-linux-build-agent:centos7-devtoolset8-x64 as dependencies_builder
...
RUN cp /vscode-web/out/vs/devpod/browser/workbench/workbench.html /vscode-web/index.html \
&& cp /vscode-web/out/vs/devpod/browser/workbench/callback.html /vscode-web/callback.html \
&& sed -i -e "s/{{VERSION}}/$CODE_QUALITY-$CODE_COMMIT/g" /vscode-web/index.html

# cli config: alises to devpod-code
RUN cp /vscode-reh-linux-x64/bin/remote-cli/devpod-code /vscode-reh-linux-x64/bin/remote-cli/code \
&& cp /vscode-reh-linux-x64/bin/remote-cli/devpod-code /vscode-reh-linux-x64/bin/remote-cli/gp-code \
&& cp /vscode-reh-linux-x64/bin/remote-cli/devpod-code /vscode-reh-linux-x64/bin/remote-cli/open
...
ARG CODE_VERSION
ARG CODE_COMMIT
LABEL "io.devpod.ide.version"=$CODE_VERSION
LABEL "io.devpod.ide.commit"=$CODE_COMMIT
