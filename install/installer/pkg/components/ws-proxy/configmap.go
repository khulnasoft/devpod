...
func configmap(ctx *common.RenderContext) ([]runtime.Object, error) {
	...
	devpodInstallationHostName := ctx.Config.Domain
	...
	devpodInstallationWorkspaceHostSuffix := fmt.Sprintf(".ws%s.%s", installationShortNameSuffix, ctx.Config.Domain)
	devpodInstallationWorkspaceHostSuffixRegex := fmt.Sprintf("\\.ws[^\\.]*\\.%s", ctx.Config.Domain)
	...
	wspcfg := config.Config{
		...
		Proxy: proxy.Config{
			...
			GitpodInstallation: &proxy.GitpodInstallation{
				...
				WorkspaceHostSuffixRegex: devpodInstallationWorkspaceHostSuffixRegex,
			},
