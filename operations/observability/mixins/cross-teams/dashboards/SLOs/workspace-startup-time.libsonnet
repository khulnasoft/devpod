...
local clusterTemplate =
  template.new(
    ...
    query='label_values(devpod_ws_manager_mk2_workspace_startup_seconds_count, %s)' % _config.clusterLabel,
    current='all',
...
local higherStartupSLOStatPanel =
  ...
  .addTarget(prometheus.target(
    |||
      (
        sum(rate(devpod_ws_manager_mk2_workspace_startup_seconds_bucket{%(clusterLabel)s=~"$cluster",type="Regular", le="128"}[30d]))
        /
        sum(rate(devpod_ws_manager_mk2_workspace_startup_seconds_count{%(clusterLabel)s=~"$cluster",type="Regular"}[30d]))
      ) - 0.95
...
local lowerStartupSLOStatPanel =
  ...
  .addTarget(prometheus.target(
    |||
      (
        sum(rate(devpod_ws_manager_mk2_workspace_startup_seconds_bucket{%(clusterLabel)s=~"$cluster",type="Regular", le="16"}[30d]))
        /
        sum(rate(devpod_ws_manager_mk2_workspace_startup_seconds_count{%(clusterLabel)s=~"$cluster",type="Regular"}[30d]))
      ) - 0.5
    ||| % _config, legendFormat='Monthly error budget remaining'
...
local workspaceStartupTimeHeatMap =
  ...
  .addTarget(prometheus.target(
    'sum(rate(devpod_ws_manager_mk2_workspace_startup_seconds_bucket{%(clusterLabel)s=~"$cluster",type="Regular"}[$__rate_interval])) by (le)' % _config,
    legendFormat='{{le}}',
    format='heatmap'
  ))
;
...
local workspaceStartupTimeQuantiles =
  ...
  .addTarget(prometheus.target(
    |||
      histogram_quantile(0.95,
        sum(rate(devpod_ws_manager_mk2_workspace_startup_seconds_bucket{%(clusterLabel)s=~"$cluster",type="Regular"}[$__rate_interval])) by (le)
      )
  ...
  .addTarget(prometheus.target(
    |||
      sum(rate(devpod_ws_manager_mk2_workspace_startup_seconds_sum{%(clusterLabel)s=~"$cluster",type="Regular"}[$__rate_interval]))
      /
      sum(rate(devpod_ws_manager_mk2_workspace_startup_seconds_count{%(clusterLabel)s=~"$cluster",type="Regular"}[$__rate_interval]))
    ||| % _config, legendFormat='avg'
  ));

{
  grafanaDashboards+:: {
    'devpod-slo-workspace-startuptime.json':
      dashboard.new(
        ...
        uid='devpod-slo-workspace-startuptime'
      )
