...
import (
	...
	cgroups_v2 "github.com/khulnasoft/devpod/common-go/cgroups/v2"
	ctntcfg "github.com/khulnasoft/devpod/content-service/api/config"
	"github.com/khulnasoft/devpod/content-service/pkg/storage"
	"github.com/khulnasoft/devpod/test/pkg/agent/daemon/api"
...
func (*DaemonAgent) VerifyRateLimitingRule(args *api.VerifyRateLimitingRuleRequest, resp *api.VerifyRateLimitingRuleResponse) error {
	*resp = api.VerifyRateLimitingRuleResponse{}
	...
	// Check if set exists.
	_, err = nftconn.GetSetByName(devpodTable, "ws-connections")
	if err != nil {
