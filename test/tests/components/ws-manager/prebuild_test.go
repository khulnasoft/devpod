...
const (
	prebuildLogPath string = "/workspace/.devpod"
		...
		Assess("it should open workspace from prebuild successfully", func(testCtx context.Context, t *testing.T, cfg *envconf.Config) context.Context {
			...
			for _, test := range tests {
				...
				t.Run(test.Name, func(t *testing.T) {
					...
					// check the folder permission is devpod
					checkFolderPermission(t, rsa, "/workspace")

					// check the files/folders permission under .git/ is devpod
...
func checkPrebuildLogExist(t *testing.T, cfg *envconf.Config, rsa *integration.RpcClient, ws *integration.LaunchWorkspaceDirectlyResult, wsRoot string) {
	...
	// we fall back to check the init task message within the /workspace/.devpod/prebuild-log-* or not
	...
	// somehow, the init task message does not exist within the /workspace/.devpod/prebuild-log-*
...
// checkFolderPermission checks the folder UID and GID is devpod
...
// checkGitFolderPermission checks the files/folders UID and GID under .git/ is devpod
