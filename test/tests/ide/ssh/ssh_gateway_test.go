...
func TestSSHGatewayConnection(t *testing.T) {
	...
	f := features.New("TestSSHGatewayConnection").
		...
		Assess("it can connect to a workspace via SSH gateway", func(testCtx context.Context, t *testing.T, cfg *envconf.Config) context.Context {
			...
			// This env var caused an incident https://www.devpodstatus.com/incidents/26gwnhcpvqqx before
			// Which was introduced by PR https://github.com/khulnasoft/devpod/pull/13822
			// And fixed by PR https://github.com/khulnasoft/devpod/pull/13858
			_ = server.SetEnvVar(ctx, &devpod.UserEnvVarValue{
				Name:              "TEST",
				RepositoryPattern: "*/*",
				Value:             "\\\"test space\\\"",
			})
